    ...
    public static async Task<bool> GeneratedTestGalleryAsync()
    {
        ...
        try
        {
            using (var process = Process.Start(processInfo))
            {
                ...
                {
                    ProjectFilesHelper.FilesToBeModified = await ProjectAnalyzerHelper.GetFilesToBeModifiedAsync(ProjectFilesHelper.TestGalleryPath);
                    ProjectFilesHelper.TargetPlatforms = await ProjectAnalyzerHelper.GetTargetPlatformsAsync(ProjectFilesHelper.TestGalleryPath);
                    await ProjectFilesHelper.ModifyFileAppShellXamlAsync();
                    await ProjectFilesHelper.ModifyFileMainPageCsAsync();
                    await ProjectFilesHelper.ModifyFileMainPageXamlAsync();
                }
            }
        }
        catch (Exception ex)
